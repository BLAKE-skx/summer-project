...
# 读取数据文件
data = pd.read_csv( r"C:\Users\10357\Desktop\毕业论文GitHub\summer-project\PER_calculate.csv")  v

# 计算每场比赛的进攻回合数
data["Possessions"] = 0.96 * (data["FGA"] + 0.44 * data["FTA"] + data["TOV"] - data["OREB"])

# 计算比赛的回合数（Pace）
data["Pace"] = data["Possessions"] / (data["MIN"] / 48)  # 假设每场比赛时长为48分钟

# 将更新后的数据写回到 CSV 文件
data.to_csv("updated_data_file.csv", index=False)  # 替换为您想要保存的文件路径
